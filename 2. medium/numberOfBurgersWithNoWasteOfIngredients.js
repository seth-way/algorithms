...
Output: [1,6]
Explantion:
To make one jumbo burger and 6 small burgers
we need 4*1 + 2*6 = 16 tomato and 1 + 6 = 7 cheese.
...
Output: []
Explantion:
Making 1 jumbo burger there will be 16 cheese remaining and making 2 small burgers
...
const numOfBurgers = (tomatoSlices, cheeseSlices) => {
    // all burgers require an even number of cheeseSlices
    if (tomatoSlices % 2 !== 0) return [];
    
    let numJumbo = 0;
    let numSmall = 0;
    
    // use the ratio of tomato to cheese to determine
    // which burger to make next
    while (cheeseSlices > 0) {
        // if ratio is less than 2 to 1, no combo is possible
        if (tomatoSlices / cheeseSlices < 2) {
            break;
        // if ratio is greater than 2 to 1, make a jumbo burger
        } else if (tomatoSlices / cheeseSlices > 2) {
            cheeseSlices -= 1;
            tomatoSlices -= 4;
            numJumbo += 1;
        // if ratio is 2 to 1, make the rest small burgers
        } else {
            numSmall += cheeseSlices;
            cheeseSlices = 0;
            tomatoSlices = 0;
        }
    }
    
    return (tomatoSlices === 0 && cheeseSlices === 0) ? [numJumbo, numSmall] : [];
};
