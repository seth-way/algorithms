...
const checkArithmeticSubarrays = (nums, l, r) => {
    // helper function determines if subArray
    // is arithmetic
    const isArithmetic = (subArr) => {
        subArr.sort((a, b) => (a - b));
        // jump must be consistant throughout subArray
        const jump = subArr[1] - subArr[0];
        for (let i = 1; i < subArr.length - 1; i += 1) {
            // check jump
            if (subArr[i + 1] - subArr[i] !== jump) {
                return false;
            }
        }
        return true;
    }
    // map subArrays created from 'l' & 'r' inputs
    return l.map((start, idx) => (isArithmetic(nums.slice(start, r[idx] + 1))));
};
