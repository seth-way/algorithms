...
const findSkyLines = (grid) => {
  const L2R = [];
  const T2B = [];
  
  grid.forEach((row, index) => {
    L2R.push(Math.max(...row));
    
    row.forEach((building, index) => {
      T2B[index] = Math.max(building, T2B[index] === undefined ? -Infinity : T2B[index]);
    });
...
const maxIncreaseKeepingSkyline = (grid) => {
  let increase = 0;
  const [L2R, T2B] = findSkyLines(grid);
  
  for (let row = 0; row < grid.length; row += 1) {
    for (let column = 0; column < grid.length; column += 1) {
      let building = grid[row][column];
      let maxL2R = L2R[row];
      let maxT2B = T2B[column];
      increase += Math.min(maxL2R, maxT2B) - building;
    }
  }
