 ...
 */
 const numberOfBeams = (bank) => {
    const numDevices = bank.map(row => row.split('').reduce((a, b) => (a + parseInt(b)), 0));
    let lasers = 0;

    for (let i = 0; i < numDevices.length; i += 1) {
        if (numDevices[i] > 0) {
            if (i + 1 < numDevices.length) {
                if (numDevices[i + 1] === 0) {
                    numDevices[i + 1] = numDevices[i];
                } else {
                    lasers += numDevices[i] * numDevices[i + 1];
                }
            }
        }
    }
