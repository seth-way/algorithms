...
You start at the point [0, 0], and you are given a destination point target = [xtarget, ytarget],
...
Explanation:
    ...
    while the ghosts located at (1, 0) and (0, 3) cannot catch up with you.
...
const escapeGhosts = (ghosts, target) => {
    const getDistanceToTarget = ([x, y]) => (
        Math.abs(target[0] - x) + Math.abs(target[1] - y)
    );
    
    const playerDistanceToTarget = getDistanceToTarget([0, 0]);
    
    for (let i = 0; i < ghosts.length; i += 1) {
        const ghostDistanceToTarget = getDistanceToTarget(ghosts[i]);
