...
You can choose a set of integers and remove all the occurrences
...
Output: 1
Explanation:
The only possible set you can choose is {7}.
 ...
 */
 const minSetSize = (arr) => {
    ...
    // loop through array & count appearences of each unique in
    for (let i = 0; i <= arr.length; i += 1) {
        if (arr[i] !== currentInt) {
            // track number of appearences for each in array
            if (firstAppearance !== i) numFreqs.push(i - firstAppearance);
            currentInt = arr[i];
            firstAppearance = i;
        }
    }
    
    numFreqs.sort((a, b) => a - b);
    
    // starting with largest freqs,
    // track how many it takes to equal half
    // of initial array length
    while(arr.length / 2 > totalIntsInSet) {
        totalIntsInSet += numFreqs.pop();
        keysInSet += 1;
    }
