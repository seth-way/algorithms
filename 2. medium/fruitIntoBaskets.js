...
const totalFruit = (tree) => {
    let maxFruits = 0;
    let fruit1; let fruit2;
    
    tree.forEach((startFruit, idx) => {
        if (startFruit !== fruit1) {
            fruit1 = startFruit;
            fruit2 = '_unassigned';
            
            let currFruits = 1;
            
            for (let i = idx + 1; i < tree.length; i += 1) {
                if (tree[i] === fruit1 || tree[i] === fruit2) {
                    currFruits += 1; //put fruit in basket, continue picking
                } else {
                    if (fruit2 === '_unassigned') {
                        fruit2 = tree[i];
                        currFruits += 1;
                    } else { // 3rd fruit found, no baskets left
                        i = tree.length;
                    }
                }
            }
