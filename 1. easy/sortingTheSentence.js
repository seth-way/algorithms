 ...
 */
 const sortSentence = (s) => {
    let result = s.split(' '), word, idx;
    
    for (let i = 0; i < result.length; i += 1) {
        // idx will be either words final idx
        // or Nan
        idx = parseInt(result[i].slice(-1)) - 1;

        // if idx is an int
        if (idx >= 0) {
            // word will remove idx from end of string
            word = result[i].slice(0, result[i].length - 1);

            // if word is in correct position, keep it
            if (idx === i) {
                result[i] = word;
            } else {
                result[i] = result[idx];
