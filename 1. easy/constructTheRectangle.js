...
const constructRectangle = (area) => {
    ...
    // solutions arranged as [L, W]
    const solutions = [[area, 1]];
    
    // find valid dimensions
    for (let width = 2; width <= sqrt; width += 1) {
        if (area % width === 0) solutions.push([area / width, width]);
    }
    
    // find solution whose length is closest to its width
    return solutions.reduce(
        (optimal, tuple) => (
            tuple[0] - tuple[1] < optimal[0] - optimal[1] ?
                tuple :
                optimal
        )
    , solutions[0]);
