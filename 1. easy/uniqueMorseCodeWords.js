...
const uniqueMorseRepresentations = (words) => {
    ...
    const wordToMorse = (word) => (
        // Map the chars of a word to their morse code counterparts.
        // Return the morse code string.
        word.split('').map((char) => (morseCodeAlphabet[char.charCodeAt() - 97])).join('')
    );
    
    const morseCodeWords = new Set();
    
    words.forEach((word) => {
        // Add each new morse code string to the set.
        morseCodeWords.add(wordToMorse(word));
    });
