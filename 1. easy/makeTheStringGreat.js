...
you can choose two adjacent characters that make the string bad and remove them.
 ...
 */
const makeGood = (s) => {    
    const badLetters = (a, b) => (
        a !== b && a.toUpperCase() === b.toUpperCase()
    );

    for (let i = 0; i < s.length - 1; i += 1) {
        if (badLetters(s[i], s[i + 1])) {
            // recursively remove letters until no bad letters are found
            return makeGood(s.slice(0, i) + s.slice(i + 2));
        }
    }
  
    // return string with only good letters
    return s;
};
